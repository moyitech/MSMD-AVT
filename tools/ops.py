...
def read_segment_image(img_file):
...
def p2dtop3d_2(p2d, z, cam, align_mat, cam_number):
    ...
    renew_xz = renew_x * z
    rexyz = np.append(np.dot(np.linalg.inv(cam.Pmat[cam_number - 1][:, 0:3]),
                             (renew_xz - np.array([cam.Pmat[cam_number - 1][:, 3]]).T)),
                      np.ones([1, renew_xz.shape[1]]), axis=0)
    rep3d = np.dot(align_mat, rexyz)
    rep3d = rep3d[0:3]
    return rep3d
...
if __name__ == '__main__':
    # img = read_image("/amax/tyut/user/wzy/workspace/KD-DIGLIM/AV163/seq01-1p-0000/seq01-1p-0000_cam1_jpg/segment/frame0001.jpg")
    img = read_segment_image("/amax/tyut/user/wzy/workspace/KD-DIGLIM/AV163/seq01-1p-0000/seq01-1p-0000_cam1_jpg/segment/frame0001.jpg")
    print(img.shape)
    # (288, 360)
