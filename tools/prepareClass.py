...
class DataMain:
    ...
    def loadGT2D(self, datasetPath, sequence, cam_number):
        ...
        dataPath = f'{datasetPath}/{sequence}/{sequence}_cam{cam_number}_jpg/{sequence}_cam{cam_number}_GT2D.txt'
...
if __name__ == '__main__':
    seqs = ['seq01-1p-0000','seq02-1p-0000','seq03-1p-0000']
    for seq in seqs:
        CFGforGCF = cfgSeqSet(seq)
        DATA = DataMain(CFGforGCF)
        res = DATA.loadCamAlign('/amax/tyut/user/wzy/workspace/KD-DIGLIM/AV163')
        # save npz
        np.savez(f'./{seq}_Pmat.npz', DATA.cam.Pmat)
        np.savez(f'./{seq}_K.npz', DATA.cam.K)
        print(DATA.cam.Pmat.shape)
        print(DATA.cam.K.shape)
